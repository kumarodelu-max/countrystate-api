...
async function fixIndexes() {
    ...
    try {
        ...
        await db.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cities_state_id ON cities(state_id);`);

        // 4. Index on translations for fast left joins
        console.log('Creating index on translations(entity_type, entity_id, locale)...');
        await db.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_translations_entity ON translations(entity_type, entity_id, locale);`);
        
        // 5. Index on states.country_id
        console.log('Creating index on states(country_id)...');
        await db.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_states_country_id ON states(country_id);`);
